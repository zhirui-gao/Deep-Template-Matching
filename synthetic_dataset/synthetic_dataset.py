...
import numpy as np
import math
from curve_points import Curves
...
def draw_lines(img, nb_lines=10):
    ...
    for i in range(num_lines):
        ...
        template_points = np.concatenate([template_points, np.array([[p1_new[0][0], p1_new[0][1]], [p2_new[0][0], p2_new[0][1]]])], axis=0)
...
def draw_polygon(img, max_sides=15):
    ...
    template_points = np.array(corners_templete).reshape(-1,2)
    ## template
    return points, template_points, template_img, np.linalg.inv(M)
...
def draw_contours(img, max_n=20):
    ...
    Parameters:
      max_sides: maximal number of sides + 1
    """
    num_corners = random_state.randint(3, max_n)
    # big

    curves = Curves(n=num_corners)
    boundary = 50
    points = curves.get_point(min_x=boundary, max_x=img.shape[1]-boundary, min_y=boundary,max_y=img.shape[0]-boundary)
    ...
    template_points = np.array(corners_templete).reshape(-1,2) # only save 10
    ## template
    return points[0:10], template_points[0:10], template_img, np.linalg.inv(M)
...
def draw_ellipses(img, nb_ellipses=40):
    ...
    for i in range(nb_ellipses):
        ...
        if (abs(ax-ax_0)/ax<0.1 and abs(ay-ay_0)/ax<0.1) or (abs(ay-ax_0)/ax<0.1 and abs(ax-ay_0)/ax<0.1):
            continue
        ...
        if ax_0==-100:
            ...
            cv.ellipse(template_img, (template_center[0][0], template_center[0][1]), (ax, ay), angle-angle_M, 0, 360, 255, -1) # clock-wise

    return np.empty((0, 2), dtype=np.int), np.empty((0, 2), dtype=np.int), template_img, np.linalg.inv(M)


def draw_star(img, nb_branches=10):
    ...
    min_dim = min(img.shape[0], img.shape[1])
    thickness = random_state.randint(5, 20)
    rad = max(random_state.rand() * min_dim / 2, min_dim / 3)
    ...
    template_points = [cal_trans_point(M, [point]) for point in points]
    template_points = np.array(template_points).reshape(-1,2)
    for i in range(1, num_branches + 1):
        cv.line(template_img, (template_points[0][0], template_points[0][1]),
                (template_points[i][0], template_points[i][1]),
                255, thickness)
    ## template
