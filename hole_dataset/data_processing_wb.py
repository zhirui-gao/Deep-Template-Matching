...
image_base_test = '/home/gzr/Data/zwb_data/HoleSeg_20221020_24094/test4337/'
image_base_train = '/home/gzr/Data/zwb_data/HoleSeg_20221020_24094/train17347/'
image_base_val = '/home/gzr/Data/zwb_data/HoleSeg_20221020_24094/val2410/'

data_type = 'data_'
W, H = 512, 512


def get_range_random(min_=0.95, max_=1.05):
...
def deal_sample(path_mask, path_image, is_ellipse, data_type, cnt, is_trianing):
    image = cv2.imread(path_image)
    image_mask = cv2.imread(path_mask, 0)
    if is_trianing is True:
        image_save_dir = '/home/gzr/Data/zwb_data/hole_wb/' + data_type + '/images/training/'
        save_data(image_save_dir, image, image_mask, is_ellipse, cnt)
    else:
        image_save_dir = '/home/gzr/Data/zwb_data/hole_wb/' + data_type + '/images/test/'
        save_data(image_save_dir, image, image_mask, is_ellipse, cnt,resize=False)
        # image_save_dir = '/home/gzr/Data/zwb_data/hole_wb/' + data_type + '_hole/images/test/'
        # save_data(image_save_dir, image, image_mask, is_ellipse, cnt, resize=False)
...
def main():
    ...
    split_num = 4
    path_mask_list = []
    #1. get all images into list
    dir_images = image_base_test + 'masks'
    ...
    # test dataset
    cnt = 0
    num_ave = len(path_mask_list)//split_num
    for i in range(len(path_mask_list)):
        path_mask = path_mask_list[i]
        path_image = path_mask.replace('/masks', '/images')
        deal_sample(path_mask, path_image, False, str(i//num_ave), cnt % num_ave,is_trianing=False) # 4336
        cnt = cnt + 1
        print(str(i//num_ave), cnt % num_ave)
        # cnt =   # mask sure the number of images is equal
