...
def process_template(mask,img,cnt,imgaug=False):

    NUM_EACH = 10000
    print(cnt // NUM_EACH+2, ": ", cnt % NUM_EACH)
    image_save_dir =os.path.join(save_dir,str(2+ cnt//NUM_EACH),'images/training')
    cnt = cnt%NUM_EACH
    ...
    if imgaug:
        aug = iaa.OneOf([
            ...
            iaa.Sharpen(alpha=1, lightness=(0.8, 1.5)),
...
for i in arr:
    ...
    for countur in contours:
        ...
        process_template(cropped_mask, cropped_img, cnt,imgaug=True)
        cnt = cnt+1
