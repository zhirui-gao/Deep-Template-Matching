...
for i in range(10):
    ...
    img = cv2.resize(img, dsize=(mask.shape[1], mask.shape[0]))
    ...
    for countur in contours:
        ...
        min_x = max(min_x - 5, 0)//2 * 2
        min_y = max(min_y - 5, 0)//2 * 2
        max_x = min((max_x + 5), W)//2 * 2
        max_y = min((max_y + 5), H)//2 * 2

        cropped_mask = mask[min_y:max_y, min_x:max_x]
        cropped_img = img[min_y:max_y, min_x:max_x]
        # if (max_y -min_y >= H_save//2-10) or (max_x-min_x >=W_save//2-10):
        #     fx=fy=1
        #     rand_len = random.randint(20, 100)
        #     if (max_y -min_y >= H_save//2-10):
        #         fx = (H_save//2 -rand_len)/(max_x - min_x)
        #     if (max_x-min_x >=W_save//2-10):
        #         fy = (W_save//2 - rand_len) / (max_x - min_x)
        #     fy = fx = min(fx, fy)
        #     cropped_mask = cv2.resize(cropped_mask,(0,0),fx=fx,fy=fy,interpolation=cv2.INTER_NEAREST)
        #     cropped_img = cv2.resize(cropped_img,(0,0),fx=fx,fy=fy)
        #     countur_x = countur_x * fx
        #     countur_y = countur_y * fy

        pad_mask = pad_middle(cropped_mask, pad_size=[H_save, W_save])
