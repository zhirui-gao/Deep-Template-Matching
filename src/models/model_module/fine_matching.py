...
class FineMatching(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.conv1 = nn.Conv2d(in_channels=self.W**2, out_channels=128, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.fc1 = nn.Linear(self.W**2 * 64, 64)
        self.fc2 = nn.Linear(64,2)
        self.tanh = nn.Tanh()
        # self.sigmoid = nn.Sigmoid()
        # self.softmax = torch.nn.Softmax(dim=1)

        # self.gridY = torch.linspace(-1, 1, steps=self.W).view(1, -1, 1, 1).expand(1, self.W, self.W, 1)
        # self.gridX = torch.linspace(-1, 1, steps=self.W).view(1, 1, -1, 1).expand(1, self.W, self.W, 1)
    ...
    def forward(self, feat_f0, feat_f1, data):
        ...
        # grid = torch.cat((self.gridX, self.gridY), dim=3).to(device)
        # grid = grid.permute(0,3,1,2) # 1,2,W,W
        ...
        sim_matrix = sim_matrix.view(M, W, W, -1).permute(0, 3, 1, 2)  # sim_matrix size is [M,ww,w,w]

        n, c, w, h = sim_matrix.size()
        sim_matrix = self.conv1(sim_matrix)
        sim_matrix = self.bn1(sim_matrix)
        sim_matrix = self.relu(sim_matrix)

        sim_matrix = self.conv2(sim_matrix)
        sim_matrix = self.bn2(sim_matrix)
        sim_matrix = self.relu(sim_matrix)
        sim_matrix = sim_matrix.contiguous().view(-1, self.W**2 * 64)


        sim_matrix = self.fc1(sim_matrix)  # [M 2] # offset
        sim_matrix = F.dropout(sim_matrix, training=self.training)
        offset = self.fc2(sim_matrix)
    ...
    def get_fine_match(self, offset, data, image0_unfold=None,image1_unfold=None):
        ...
        #     plt.savefig('./image/'+ str(i)+'_raw_img', bbox_inches='tight', pad_inches=0)
        #     plt.close()
        #
        #     plt.matshow(image1_unfold[i].reshape(self.W, self.W).detach().cpu().numpy())
        #     plt.savefig('./image/' + str(i) + '_id_edge', bbox_inches='tight', pad_inches=0)
        #     plt.close()


        # if image0_unfold is not None:
