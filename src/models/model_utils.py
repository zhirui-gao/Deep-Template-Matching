...
class SuperPointNet_process(object):
    ...
    def pred_soft_argmax(self, labels_2D, heatmap):
        ...
        device = self.device
        from src.utils.losses import norm_patches

        outs = {}
        # extract patches
        from src.utils.losses import extract_patches
        from src.utils.losses import soft_argmax_2d
        ...
        # predict offsets
        from src.utils.losses import do_log
    ...
    def sample_desc_from_points(coarse_desc, pts, cell_size=8):
        ...
        if pts.shape[1] == 0:
            ...
            desc = torch.nn.functional.grid_sample(coarse_desc, samp_pts,
                                                   align_corners=True)  # tensor [batch_size(1), D, 1, N]
    ...
    def heatmap_to_nms(self, heatmap, tensor=False, boxnms=False):
        ...
        from src.utils.var_dim import toNumpy
        heatmap_np = toNumpy(heatmap)
        ## heatmap_nms
        if boxnms:
            from src.utils.utils import box_nms
    ...
    def heatmap_nms(heatmap, nms_dist=4, conf_thresh=0.015):
        ...
        from src.utils.utils import getPtsFromHeatmap
    ...
    def batch_extract_features(self, desc, heatmap_nms_batch, residual):
        ...
        for i in range(batch_size):
            ...
            # get random shuffle
            from src.utils.utils import crop_or_pad_choice
